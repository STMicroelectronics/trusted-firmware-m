...
struct mmc_ops {
	...
	int (*set_ios)(const struct device *dev,
		       unsigned int clk_rate, unsigned int width);
	int (*prepare)(const struct device *dev, int lba,
		       uintptr_t buf, size_t size, bool dir_read);
	int (*read)(const struct device *dev, int lba,
		    uintptr_t buf, size_t size);
	int (*write)(const struct device *dev, int lba,
		     const uintptr_t buf, size_t size);
	struct mmc_dev_info * (*get_dev_info)(const struct device *dev);
};
