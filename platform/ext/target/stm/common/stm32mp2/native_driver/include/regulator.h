...
typedef int (*regulator_show_t)(const struct device *dev);
...
/**
 * @brief Get a regulator device by "name-supply" properpty
 *
 ...
 *
 * @code{.c}
 *     DT_DEV_REGULATOR_SUPPLY(DT_NODELABEL(foo), va) //regulator device on vdd
 *     DT_DEV_REGULATOR_SUPPLY(DT_NODELABEL(foo), vb) //regulator device on v3v
 * @endcode
 *
 * @param node_id node identifier
 ...
 * @param name lowercase-and-underscores name of a name-supply property
 * @return reference on regulator device of "name-supply"
 */
#define DT_INST_DEV_REGULATOR_SUPPLY(inst, name)			\
	DT_DEV_REGULATOR_SUPPLY(DT_DRV_INST(inst), name)
