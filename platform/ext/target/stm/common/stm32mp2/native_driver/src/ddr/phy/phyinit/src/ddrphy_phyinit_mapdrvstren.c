...
int ddrphy_phyinit_mapdrvstren(int drvstren_ohm, enum drvtype targetcsr)
{
	...
	if ((targetcsr == DRVSTRENFSDQP) || (targetcsr == DRVSTRENFSDQN)) {
		...
		if (drvstren_ohm == 120) {
			...
			DDR_ERROR("%s userinputadvanced.atximpedance %d Ohms value is not valid.\n",
			      __func__, drvstren_ohm);
			DDR_ERROR("Valid values are: 20, 24, 30, 40, 60 and 120 Ohms.\n");
		}

	} else {
		DDR_ERROR("targetcsr = %u, in %s, is not valid.\n",
		      targetcsr, __func__);
	}
