 ...
 * \note This function doesn't check if dev is NULL.
 */
enum mpu_armv8m_error_t mpu_armv8m_region_enable(struct mpu_armv8m_dev_t *dev,
...
/*
 * Loop over each element of mpu region table.
 */
#define for_each_mpu_region(rgt, rg, nr, i)				\
	for (i = 0, rg = ((struct mpu_armv8m_region_cfg_t *)rgt);	\
	     i < (nr);							\
	     i++, rg = rg + 1)
