...
static inline void sendmsg(void *msg, size_t msg_len) {
        (*(uint32_t*)(S_RSS_SHMEM_ADDR))= msg_len;
	memcpy((uint8_t*)(S_RSS_SHMEM_ADDR + 4), msg, msg_len);
}

static inline void recvmsg(void *msg, size_t * msg_len) {
	*msg_len = *msg_len >= (*(uint32_t*)(S_RSS_SHMEM_ADDR)) ? (*(uint32_t*)(S_RSS_SHMEM_ADDR)): *msg_len;
	memcpy((uint8_t *)msg, (uint8_t*)(S_RSS_SHMEM_ADDR + 4), *msg_len);
}
...
out_return_err:
	...
	sendmsg((uint8_t *)&reply, reply_size);
